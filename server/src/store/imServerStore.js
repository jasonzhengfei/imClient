...
export const imServerStore = new Vuex.Store({
    ...
    mutations: {
        setTransferUserSeatList(state, transferUserSeatList) {
            console.log(transferUserSeatList)
            state.transferUserSeatList = transferUserSeatList
        },
        setChatName(state, chatName) {
            console.log("chatName")
            console.log(chatName)
            state.serverChatEn.serverChatName = chatName;
        },
        setClientChatEn(state, clientChatEn) {
            console.log("setClientChatEn", clientChatEn)
            state.clientChatEn = clientChatEn
        },
    ...
    actions: {
        ...
        addClientChat: function (context, { newChatEn }) {
            context.dispatch('getChatEnByChatId', { clientChatId: newChatEn.clientChatId }).then((chatEn) => {
                if (chatEn == null) {
                    ...
                    context.state.currentChatEnlist.unshift(newChatEn);

                }
                console.log(chatEn, "chatEn")
                console.log(newChatEn, "newChat")
                var params = { clientChatId: newChatEn.clientChatId }
                // var params={phoneNum:newChatEn.phone}
                console.log(params, "paramsss")
                http.post({
                    url: '/backend2/Api/WebChat/getTalk',
                    ...
                    successCallback: (rs) => {
                        ...
                        for (var i = 0; i < rs.length; i++) {
                            ...
                            var role = rs[i].role
                            var contentType = rs[i].contentType
                            var fileUrl = rs[i].fileUrl
                            var serverChatName = rs[i].serverChatName
                            var sender = rs[i].sender
                            var createTime=rs[i].createTime
                            if (content == "noanswer") {
                                content = "您的问题我还在学习当中，您可以转人工"
                                role = "server"
                            }
                            if (role == "client") {
                                role = "server"
                            }
                            if (content == "noanswer" || content == "customerSelect") {
        ...
        addChatMsg: function (context, { clientChatId, msg, successCallback }) {
            ...
            context.dispatch('getChatEnByChatId', { clientChatId: clientChatId }).then((chatEn) => {
                ...
                var ms = m < 10 ? "0" + m : m
                var ss = s < 10 ? "0" + s : s
                document.title = hs + ":" + ms + ":" + ss + "--" + msg.content
        ...
        CLIENT_ON: function (context, payload) {
            ...
            context.dispatch('addClientChat', {
                newChatEn: {
                    clientChatId: context.state.clientChatEn.clientChatId,
                    clientChatName: context.state.clientChatEn.clientChatName,
                    avatarUrl: context.state.clientChatEn.avatarUrl,
                    projName: context.state.clientChatEn.projName,
                    port: context.state.clientChatEn.port,
                }
            });
        },
    ...
    getters: {
        ...
        transferUserSeatList: function (state) {
            return state.transferUserSeatList;
        },
        /**
         * 选中的chat含有新消息
         */
