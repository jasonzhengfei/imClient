    ...
    <div class="imRecord-wrapper">
        <header class="header">
            <div class="kf-info-wrapper" style="display:inline">
                ...
                <span style="float:left;margin-top:14px"> 当前状态:</span>
                 <el-radio-group @change="changeStatus" style="float:left;margin-left:5px;margin-top:8px;display:inline" v-model="status" size="small">
...
export default {
    ...
    methods: {
        changeStatus(){
            ...
            if(this.status=="在线"){
                this.$emit('select',{chatEn:this.$store.imServerStore.getters.serverChatEn})
            }else{
                this.$emit('unselect',{chatEn:this.$store.imServerStore.getters.serverChatEn})
            }
        },
        /**
         * 选中当前列表的chat
