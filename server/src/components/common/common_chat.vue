  ...
  <div class="common_chat-wrapper">
            ...
            <div v-for="(item, index) in chatInfoEn.msgList" :key="index">
              ...
              <div v-if="item.role == 'sys'" class="item sys">
                ...
                <div v-if="item.contentType == 'text'" class="text-content">
                  <p style="color:#b0b0b0" v-show="!showHistory">{{ fomatDate(item.createTime) }}</p>
                  <p>{{ item.content }}</p>
                </div>
              </div>
              <!-- 客户、客服 -->
              <div v-else class="item" :class="{
                ...
                <div class="info-wrapper" :class="item.state">
                  ...
                  <div class="avatar-wrapper">
                    ...
                    <p v-show="item.sender == 'agent'">{{ item.serverChatName }}</p>
                  </div>
                  <!-- 1)文本类型 -->
        ...
        <div>
          ...
          <div class="opr-wrapper">
            ...
            <a class="item" href="javascript:void(0)" @click="fileUpload_click('file')">
              <i class="iconfont fa fa-picture-o"></i>
            </a>
            <form method="post" enctype="multipart/form-data">
              <input type="file" accept="image/*" name="uploadFile" id="common_chat_opr_fileUpload" style="
                  display: none;
                  position: absolute;
                  left: 0;
                  top: 0;
            ...
            <el-upload style="float:left;" class="upload-demo" action="backend2/api/file/upload" :multiple="false"
              :file-list="fileList" :on-success="uploadSucessFile">
              <i style="font-size: 18px;margin-top:2px;color:#aaa" class="iconfont fa fa-file-o"></i>
            </el-upload>
            <i v-show="historyFlag == true" @click="showHistory()" style="font-size: 20px;margin-left:13px;color:#aaa"
              class="iconfont fa fa-book"></i>
            <!-- <span style="
                padding: 5px;
                font-size: 14px;
                background: #1072b5;
          ...
          <div class="input-wrapper">
            <div maxlength="500" class="inputContent common_chat_emoji-wrapper-global" id="common_chat_input"
              ref="chatContent" contenteditable="true" style="font-size: 20px" @paste.stop="inputContent_paste"
              @drop="inputContent_drop"></div>
          </div>
          <!-- 发送按钮 -->
          <!-- @paste.stop="inputContent_paste" 
                       ...
                       -->
          <el-button type="primary" size="small" class="send-btn" :class="chatInfoEn.state"
            @click="sendText()">发送</el-button>
        </div>
        <!-- 离线 -->
    ...
    <el-dialog title :visible.sync="imgViewDialogVisible" class="imgView-dialog" :modal="false">
      ...
      <div class="main" @mousewheel="bbimg(this)">
        <img class="img" :src="imgViewDialog_imgSrc" :style="test" />
      </div>
    </el-dialog>
    <el-dialog :visible.sync="TransferDialogVisible" width="50%">
      <div>
        <div class="imTransfer-wrapper">
          <main class="main">
            <el-radio-group v-model="selectedServerChatId" class="itemt-group">
              <div class="itemt" v-for="(item, index) in serverChatList" :key="index">
                <el-radio-button :label="item.serverChatId">{{ item.serverChatName }}</el-radio-button>
              </div>
            </el-radio-group>
          </main>
          <footer class="footer">
            <!-- //:disabled="selectedServerChatId == ''" -->
            <el-button type="primary" @click="submit">转接</el-button>
          </footer>
        </div>
      </div>
    </el-dialog>
...
export default {
  ...
  computed: {
    storeTransferUserSeatList() {
      return this.$store.imServerStore.getters.transferUserSeatList;
    },
    storeServerChatEn() {
  ...
  methods: {
    ...
    showHistory() {
      ...
      this.$http.post({
        url: '/backend2/api/WebChat/getChatByPhone',
        params: formData,
        successCallback: (rs) => {
          console.log('rss');
          console.log(rs);
    ...
    submit() {
      console.log(this.selectedServerChatId)
      console.log("this.storeServerChatEn", this.storeServerChatEn)
      var serverChatName = "";
      var avatarUrl = "";
      for (var i = 0; i < this.serverChatList.length; i++) {
        if (this.serverChatList[i].serverChatId == this.selectedServerChatId) {
          serverChatName = this.serverChatList[i].serverChatName
          avatarUrl = this.serverChatList[i].avatarUrl
          break;
        }
      }
      var msg = { avatarUrl: avatarUrl, serverChatName: serverChatName, serverChatId: this.storeServerChatEn.serverChatId, transServerChatId: this.selectedServerChatId }

      this.$emit('transNewSeat', {
        msg: msg,
      ...
      this.$emit('addChatMsg', {
        role: 'sys',
        contentType: 'text',
        content: ' 已转至' + serverChatName + "坐席,本轮通话已结束",
      });


      this.$emit('transReSetClientChatEn', this.storeServerChatEn)
      this.TransferDialogVisible = false;
    },
    transfer() {

      this.$emit('transferUserSeat', {
        successCallbcak: function () {
          console.log("transferUserSeatCallBack")
          console.log(this.storeTransferUserSeatList)
          this.serverChatList = this.storeTransferUserSeatList
    ...
    TransferShow() {
      this.serverChatList = []
      console.log(this.storeTransferUserSeatList, "storeTransferUserSeatList")
      console.log(this.storeServerChatEn, "this.storeServerChatEn")
      for (var i = 0; i < this.storeTransferUserSeatList.length; i++) {
        if (this.storeTransferUserSeatList[i].serverChatId != this.storeServerChatEn.serverChatId) {
          this.serverChatList.push(this.storeTransferUserSeatList[i])
        }
      }
      console.log(this.storeTransferUserSeatList, "storeTransferUserSeatList")
     ...
     */
     setInputDiv:function(value){
      console.log("setInputDiv1", value)
      value = this.getqqemojiEmoji(value);
      console.log("setInputDiv2", value)
      var old=document.getElementById('common_chat_input').innerHTML
      console.log("old",old)
      old=old+value
      console.log("old",old)
      document.getElementById('common_chat_input').innerHTML=old
      this.setInputContentByDiv()
     },
    setInputDiv241011: function (value) {
      console.log("setInputDiv", value)
    ...
    inputContent_paste: function (e) {
      console.log('inputContent_paste');
      var self = this;
      var isImage = false;
      ...
      //             formData.append('uploadFile', file);
      //             this.$http.uploadFile({
      //                 url: 'http://192.168.3.220:8013/api/File/upload',
      //                 params: formData,
      //                 successCallback: (rs) => {
      //                     console.log("rs")
      //                     console.log(rs[0])
    ...
    sendMsg: function (msg) {
      console.log("sendMsg", this.storeServerChatEn)
      if (this.storeServerChatEn.clientChatId == "00000000") {
        return;
      }
      console.log('sendMsg00');
      console.log(msg);
      var self = this;
      // 1.传递
      this.$emit('sendMsg', {
